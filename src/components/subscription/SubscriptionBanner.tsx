...
import { AlertTriangle, Clock, Ban, CreditCard, Sparkles } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useNavigate } from "react-router-dom";
import { cn } from "@/lib/utils";

export function SubscriptionBanner() {
  const { subscription, loading, isTrial, isExpired, isBlocked, isTrialExpiring, trialDaysRemaining, planLabel, canWrite } = useSubscription();
  ...
  // Trial active but not expiring soon
  ...
  return (
    <div className={cn(
      "border rounded-lg px-4 py-3 flex items-center gap-3 flex-wrap",
      bannerConfig.bg
    )}>
      ...
      {bannerConfig.showCTA && (
        <Button size="sm" variant="default" className="gap-1.5 shrink-0" onClick={() => navigate("/")}>
